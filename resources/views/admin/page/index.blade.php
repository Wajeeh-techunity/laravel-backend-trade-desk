    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-12">
                        <h1 class="text-center">Manage Pages</h1>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <a href="{{route('create.page')}}" class="btn btn-success"><i class="fas fa-plus-square"> Add New Page</i></a>
                                   ...
                                   @foreach($pages as $page)
                                        <tr>
                                            <td class="text-center singleCheck">
                                                <input type="checkbox" name="selected" class="singleChk"  value="{{$page->id}}" />
                                            </td>
                                            <td>{{$page->title}}</td>
                                            <td>{{$page->short_title}}</td>
                                            <td><a href="{{URL::to($page->slug)}}">{{URL::to($page->slug)}}</a></td>
                                            <td>@if($page->status == 1) <span class="badge badge-success"> Active </span>@else  <span class="badge badge-danger"> In Active </span> @endif</td>
                                            <td>{{$page->findUser->name}}</td>
                                            <td>{{$page->created_at}}</td>
                                            <td>
                                                <div class="btn-group btn-group-sm">
                                                    <a href="{{route('edit.page', ['id' => $page->id])}}" class="btn btn-info"><i class="fas fa-edit"></i></a>
                                                    <button class="btn btn-danger delete_page" data-id="{{$page->id}}"><i class="fas fa-trash"></i></button>
                                                </div>
                                            </td>
                                        </tr>
                                   @endforeach
...
@section('js')
    ...
    <script>
        ...
        $(document).on('click','.delete_page', function(e){
            ...
            removeByIds(ids,'{{route('delete.page')}}');
        })
